...
- #### 这是一个未改主体的可高度自定义的xyhelper旗下的share二开。集注册、登录、在线下单、分离plus会员/普通会员、集成claude并获取剩余次数、邀请返利机制、二级分销系统、优惠券发放、激活码发放、公告管理、虚拟车队、审计限流等实用功能于一体。

- #### 本项目基于share-server原版，必须使用xy接入点。因未改动主体，原版share的全部功能都可用，包括限制多设备登录、无感换车、本地保留对话记录等等。
